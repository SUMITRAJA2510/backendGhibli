...
export async function convertImageToValidSize(file, targetSize = 1024, targetKB = 200) {
    return new Promise((resolve, reject) => {
        ...
        img.onload = () => {
            ...
            // Try reducing quality until size ≈ targetKB
            const tryCompress = (quality) => {
                canvas.toBlob(
                    (blob) => {
                        if (!blob) return reject("Canvas is empty");
                        if (blob.size / 1024 <= targetKB || quality <= 0.3) {
                            // Stop if below target size or too low quality
...
const PhotoToImageSection = () => {
    ...
    const handleFileChange = async (file) => {
        if (file && file.type.startsWith("image/")) {
            const resizedImage = await convertImageToValidSize(file);

            if (resizedImage && resizedImage.type === "image/jpeg") {
                setUploadedFile(resizedImage);
                setUploadedImage(URL.createObjectURL(resizedImage));
    ...
    const handleGenerate = async () => {
        ...
        console.log(uploadedFile, uploadedImage, prompt, formData, 'shubh data');
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    {uploadedImage ? (
                        ...
                        <div>
                            ...
                            <button onClick={onBrowseClick}
                                    className="bg-gray-200 text-gray-800 font-semibold py-2 px-4 rounded-lg hover:bg-gray-300 transition-colors">Browse
                                files
                            </button>
                            <input ref={fileInputRef} onChange={(e) => handleFileChange(e.target.files[0])} type="file"
                ...
                {!generatedImage && (
                    <>
                        <div className="mt-6 space-y-4">
                            <div>
                                <label htmlFor="prompt-photo" className="text-md font-semibold mb-2 block">Additional
                                    Details</label>
                                <textarea id="prompt-photo" className="w-full p-3 border border-gray-300 rounded-lg"
                                          rows="2" placeholder="Add any specific details or enhancements..."
                                          value={prompt} onChange={(e) => {
                                    setPrompt(e.target.value)
                                }}></textarea>
                            </div>
                        </div>
                        <button onClick={(event) => handleGenerate(event)} disabled={isCreateDisabled}
            ...
            <div
                className="bg-white/80 backdrop-blur-sm p-8 rounded-2xl shadow-lg flex flex-col justify-center items-center">
                <div
                    className="w-full h-full flex justify-center items-center border-2 border-gray-200 rounded-xl bg-gray-50 min-h-[400px]">
                    {isLoading ? (<Spinner/>) : generatedImage ? (<img src={generatedImage} alt="Final Ghibli art"
                                                                       className="max-h-[32rem] w-auto rounded-lg object-contain"/>) : (
                        <p className="text-center text-gray-500 max-w-sm">Your generated Ghibli art will appear
                            here.</p>
                    )}
                </div>
            </div>
